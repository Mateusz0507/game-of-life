...
from constants import DTYPE, CELL_SIZE, SIDEBAR_WIDTH, SCREEN_WIDTH, SCREEN_HEIGHT
...
class Board:
    def __init__(self, width, height):
        ...
        self.c_width, self.c_height = self.calculate_cell_coords(width, height)
    ...
    def change_size(self, width, height, fullscreen):
        if fullscreen:
            new_c_width, new_c_height = self.calculate_cell_coords(
                SCREEN_WIDTH, SCREEN_HEIGHT
            )
        else:
            new_c_width, new_c_height = self.calculate_cell_coords(width, height)
        ...
        for x in range(self.c_width):
            for y in range(self.c_height):
                ...
                for coords in [
                    ...
                    if 0 <= coords[0] < self.c_width and 0 <= coords[1] < self.c_height:
    ...
    def draw_selected_cell(self, surface, mouse_x, mouse_y):
        ...
        pg.draw.rect(
            surface,
            RED,
            [x * CELL_SIZE + SIDEBAR_WIDTH, y * CELL_SIZE, CELL_SIZE, CELL_SIZE],
            2,
        )
